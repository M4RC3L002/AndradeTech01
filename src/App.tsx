...
function RevenueScreen({ sales = [], orders = [], isDark, onOpenMenu }: { sales: Sale[]; orders: Order[]; isDark: boolean; onOpenMenu: () => void }) {
  ...
  const presetEnd = period === 'month' ? new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59) : new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59)
  const selectedEnd = period === 'custom' ? (fromInputDate(customEnd, true) || new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59)) : presetEnd
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <Topbar title="Faturamento" isDark={isDark} onOpenMobileMenu={onOpenMenu} rightContent={<button type="button" onClick={() => setShowReport(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-[#0066FF] to-[#8A2BE2] px-3 py-1.5 text-xs font-bold text-white shadow hover:opacity-95"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2"/><path d="M6 14h12v8H6z"/></svg><span className="hidden sm:inline">Gerar Relatório</span><span className="sm:hidden">PDF</span></button>}/>
      <div className="flex-1 overflow-y-auto p-4 sm:p-5">
